...
public:
	...
	void burstNodes();
	int burstNode(GNodeptr); //this will start the chain reaction
...
void ChainReaction::play()
{
	...
	do
	{
		...
		{
			cout << "Please choose a valid index: ";
			cin >> index;
		}
...
int ChainReaction::gameLeft()
{
	...
	{
		if (this->gmap[index]->count < this->gmap[index]->total)
			return 1;
	}
	return 0;
}
...
int ChainReaction::playerLeft(PNodeptr pnode)
{
	for (int index = 0; index < this->gameSize * this->gameSize; index++)
	{
		if (this->gmap[index]->player == pnode)
			return 1;
	}
	return 0;
}
...
void ChainReaction::markIndex(PNodeptr pnode, int index)
{
	...
	//this->printGame();
	if (gnode->count == gnode->total)
	{
		this->nextBurstNodes.push(gnode);
		if (!this->burstingNodesAlready)
			this->burstNodes(); //start the chain reaction
	}
}

void ChainReaction::burstNodes()
{
	this->burstingNodesAlready = 1;
	do
	{
		if (!this->burstNode(this->nextBurstNodes.top()))
			return;
		this->nextBurstNodes.pop();
	} while (!this->nextBurstNodes.empty());
	this->burstingNodesAlready = 0;
}

//the chain reaction
int ChainReaction::burstNode(GNodeptr gnode)
{
	PNodeptr player = this->firstPlayer; //check if the player has not won already
	int flag = 1;
	do
	{
		if (this->playerLeft(player) && gnode->player != player)
		{ //there must a opponent left in the game to proceed further
			flag = 0;
			break;
		}
		player = player->next;
	...
	if (flag)
		return 0; /* MY FAVOURITE PART OF THE CODE */ //if the player has already won stop the chain reaction
	...
	if (gnode->left != NULL)
		this->markIndex(player, gnode->left->index); //burst to the left
	if (gnode->right != NULL)
		this->markIndex(player, gnode->right->index); //burst to the right
	if (gnode->up != NULL)
		this->markIndex(player, gnode->up->index); //burst upwards
	if (gnode->down != NULL)
		this->markIndex(player, gnode->down->index); //burst downwards
	return 1;
}
...
{
	if (gmap[0]->player == NULL)
		return 0;
	else if (gmap[gameSize - 1]->player == NULL)
		return gameSize - 1;
	else if (gmap[gameSize * gameSize - gameSize]->player == NULL)
		return gameSize * gameSize - gameSize;
	else if (gmap[gameSize * gameSize - 1]->player == NULL)
		return gameSize * gameSize - 1;
	else
